...
exports.msgErr = function() {
  let errores = Object.values(myErr);
...
let { User } = require("../src/data");
exports.chat_gpt = async (jid, budy) => {
  try {
      const openai = new OpenAI({
        apiKey: Config.OPEN_AI_KEY || console.log('Err ApikEy'),
      });
      const conversationHistory = await User.getConversationHistory(jid);
      ...
      await User.addToConversationHistory(jid, "user", budy);
      await User.addToConversationHistory(jid, "assistant", respuestaDelChatbot);
  
      return respuestaDelChatbot
    } catch (e) {
      console.error(e)
    }
}
